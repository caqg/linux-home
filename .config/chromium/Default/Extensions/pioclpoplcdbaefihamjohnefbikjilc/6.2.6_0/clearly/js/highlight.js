...
_inner=_inner.replace(/<span([^>]*?)>/gi,'');_inner=_inner.replace(/<\/span>/gi,'');$H.settings.assignInnerHTML(_parents[i],_inner);}};$H.highlight__getDeepestTextNode=function(_node)
...
{var _s=$H.document.createElement('span');$H.settings.assignInnerHTML(_s,_e.innerHTML);_e.parentNode.replaceChild(_s,_e);_parents_to_clean.push(_s.parentNode);});$H.highlight__deleteSpansFromParents(_parents_to_clean);};$H.enable=function()
{if($H.enabled){return;}
