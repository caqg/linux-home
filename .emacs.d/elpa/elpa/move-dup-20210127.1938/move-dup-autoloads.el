...
If called interactively, toggle `Move-Dup mode'.  If the prefix
...
With prefix ARG, enable Global Move-Dup mode if ARG is positive; otherwise,
disable it.  If called from Lisp, enable the mode if ARG is omitted or nil.

Move-Dup mode is enabled in all buffers where `move-dup-on' would do it.
